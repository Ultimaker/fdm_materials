...
class MaterialProfile:
    ...
    @classmethod
    def from_file(cls, p: Path) -> 'MaterialProfile':
        with p.open("rb") as f:
...
class MaterialProfilesValidator:
    ...
    def __init__(self, xsd_file: Path) -> None:
        self._schema = self.load_schema(xsd_file)
        self._guids_seen: Dict[str, Path] = dict()

    @staticmethod
    def load_schema(xsd_file: Path) -> etree.XMLSchema:
...
def validate_files(xsd_file: Path, files_to_check: Iterable[Path]) -> bool:
    ...
    for f in files_to_check:
        try:
            ...
            profile = MaterialProfile.from_file(f)
...
def main():
    ...
    success = validate_files(args.xsd, files_to_check)
